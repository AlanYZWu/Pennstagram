...
public class SimpleManipulations {
    ...
    public static PixelPicture rotateCCW(PixelPicture pic) {
        int w = pic.getWidth(); // # of columns in original
        int h = pic.getHeight(); // # of rows in original

        Pixel[][] src = pic.getBitmap(); // Original picture
        Pixel[][] tgt = new Pixel[w][h]; // Rotated picture
    ...
    public static PixelPicture border(
        ...
        for (int row = 0; row < tgt.length; row++) {
            for (int col = 0; col < tgt[0].length; col++) {
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        Pixel[][] inverted = pic.getBitmap(); // Original image to be inverted

        for (int row = 0; row < inverted.length; row++) {
            for (int col = 0; col < inverted[0].length; col++) {
                int r = inverted[row][col].getRed(); // Red value of original pic
                int g = inverted[row][col].getGreen(); // Green value of original pic
                int b = inverted[row][col].getBlue(); // Blue value of original pic

                inverted[row][col] = new Pixel(255 - r, 255 - g, 255 - b); // Inverts color
            }
        }

        return new PixelPicture((inverted));
    }
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        Pixel[][] gray = pic.getBitmap(); // Original image to be converted to gray

        for (int row = 0; row < gray.length; row++) {
            for (int col = 0; col < gray[0].length; col++) {
                int r = gray[row][col].getRed(); // Red value of original
                int g = gray[row][col].getGreen(); // Green value of original
                int b = gray[row][col].getBlue(); // Blue value of original
                int avg = (int) Math.round((r + g + b) / 3.0); // Average pixel color

                gray[row][col] = new Pixel(avg, avg, avg);
            }
        }
        return new PixelPicture(gray);
    }
    ...
    public static PixelPicture scaleColors(
        ...
        Pixel[][] scaled = pic.getBitmap(); // Original image to be scaled

        for (int row = 0; row < scaled.length; row++) {
            for (int col = 0; col < scaled[0].length; col++) {
                int r = (int) Math.round(rfactor * scaled[row][col].getRed()); // Scaled red val
                int g = (int) Math.round(gfactor * scaled[row][col].getGreen()); // Scaled green val
                int b = (int) Math.round(bfactor * scaled[row][col].getBlue()); // Scaled blue val

                scaled[row][col] = new Pixel(r, g, b);
            }
        }
        return new PixelPicture(scaled);
    }
    ...
    public static PixelPicture alphaBlend(
        ...
        if (pic.getHeight() != f.getHeight() || pic.getWidth() != f.getWidth()) {
            ...
            Pixel[][] blended = new Pixel[pic.getHeight()][pic.getWidth()]; // Blended image
            Pixel[][] first = pic.getBitmap();
            Pixel[][] second = f.getBitmap();

            for (int row = 0; row < first.length; row++) {
                for (int col = 0; col < first[0].length; col++) {
                    int avgR = weightedAverage(alpha, first[row][col].getRed(),
                            second[row][col].getRed());
                    int avgG = weightedAverage(alpha, first[row][col].getGreen(),
                            second[row][col].getGreen());
                    int avgB = weightedAverage(alpha, first[row][col].getBlue(),
                            second[row][col].getBlue());

                    blended[row][col] = new Pixel(avgR, avgG, avgB);
                }
            }
            return new PixelPicture(blended);
        }
    }
