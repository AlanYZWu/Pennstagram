...
public class AdvancedManipulations {
    ...
    public static PixelPicture adjustContrast(
        ...
        int intensity = 0; // Average intensity
        Pixel[][] img = pic.getBitmap(); // Original pic

        // Totals color values
        for (int row = 0; row < img.length; row++) {
            for (int col = 0; col < img[0].length; col++) {
                intensity += img[row][col].getRed();
                intensity += img[row][col].getGreen();
                intensity += img[row][col].getBlue();

            }
        }

        // Averages intensity
        intensity = (int) Math.round(intensity / 3.0 / (img.length * img[0].length));

        // Normalize, scale, and change pixels
        for (int row = 0; row < img.length; row++) {
            for (int col = 0; col < img[0].length; col++) {
                int r = (int)Math.round((img[row][col].getRed() - intensity) * multiplier);
                int g = (int)Math.round((img[row][col].getGreen() - intensity) * multiplier);
                int b = (int)Math.round((img[row][col].getBlue() - intensity) * multiplier);

                img[row][col] = new Pixel(r + intensity, g + intensity, b + intensity);
            }
        }

        return new PixelPicture(img);
    }
    ...
    public static PixelPicture reducePalette(PixelPicture pic, int numColors) {
        int h = pic.getHeight(); // # or rows
        int w = pic.getWidth(); // # of columns
        Pixel[][] img = pic.getBitmap(); // Original image
        ColorMap colors = new ColorMap(); // Color Map

        // Adds frequencies to color map
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                int freq = 1;
                if (colors.contains(img[row][col])) {
                    freq = colors.getValue(img[row][col]) + 1;
                }
                colors.put(img[row][col], freq);
            }
        }

        Pixel[] pallet = colors.getSortedPixels();

        // Changes pixels
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                int diff = Integer.MAX_VALUE;
                Pixel closest = pallet[0];

                for (int i = 0; i < numColors; i++) {
                    if (diff > pallet[i].distance(img[row][col])) {
                        diff = pallet[i].distance((img[row][col]));
                        closest = pallet[i];
                    }
                }
    ...
    public static PixelPicture blur(PixelPicture pic, int radius) {
        Pixel[][] img = pic.getBitmap(); // Original picture
        int h = pic.getHeight(); // # of rows
        int w = pic.getWidth(); // # of columns
        Pixel[][] blurry = new Pixel[h][w]; // Blurry image
            ...
            for (int col = 0; col < w; col++) {
                ...
                for (int i = row - radius; i <= row + radius; i++) {
                    for (int j = col - radius; j <= col + radius; j++) {
                        if (i >= 0 && j >= 0 && i < h && j < w) {
                            rSum += img[i][j].getRed();
                            gSum += img[i][j].getGreen();
                            bSum += img[i][j].getBlue();
                            pixelCount++;
                        }
                    }
                }

                int rAvg = (int) Math.round(rSum / pixelCount);
                int gAvg = (int) Math.round(gSum / pixelCount);
                int bAvg = (int) Math.round(bSum / pixelCount);
                blurry[row][col] = new Pixel(rAvg, gAvg, bAvg);
            }
        }
        return new PixelPicture(blurry);
    }
