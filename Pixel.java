...
public class Pixel implements Comparable<Pixel> {
    ...
    public Pixel(int r, int g, int b) {
        this.red = pixelHelper(r) ;
        this.green = pixelHelper(g);
        this.blue = pixelHelper(b);
    }

    private static int pixelHelper(int x) {
        if (x > 255) {
            return 255;
        } else {
            return Math.max(x, 0);
    ...
    public Pixel(int[] c) {
        if (null == c) {
            ...
            this.red = pixelHelper(c[0]);
            this.green = 0;
            this.blue = 0;
        } else if (c.length == 2) {
            this.red = pixelHelper(c[0]);
            this.green = pixelHelper(c[1]);
            this.blue = 0;
        } else {
            this.red = pixelHelper(c[0]);
            this.green = pixelHelper(c[1]);
            this.blue = pixelHelper(c[2]);
        }
    }
    ...
    public int[] getComponents() {
        return new int[] {this.red, this.green, this.blue}; // Implement this method
    }
    ...
    public int distance(Pixel px) {
        if (null == px) {
            ...
            return Math.abs(this.red - px.getRed()) + Math.abs(this.green - px.getGreen()) +
                    Math.abs(this.blue - px.getBlue()); // Implement this method
        }
    }
    ...
    public String toString() {
        return "(" + this.red + ", " + this.green + ", " + this.blue + ")"; // Implement this method
    }
    ...
    public boolean sameRGB(Pixel px) {
        if (null == px) {
            ...
            return Arrays.equals(this.getComponents(), px.getComponents()); // Implement this method
        }
    }
